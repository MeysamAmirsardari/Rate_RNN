...
Entry point for the live model0 demo.
    ...
    python -m live_demo --preset dynamic2            ~10 s forgetting horizon
    ...
    python -m live_demo --selftest                   headless validation
    python -m live_demo --snapshot out.png --source wav --wav a.wav
...
# project root on sys.path for `python live_demo/__main__.py` too
_ROOT = Path(__file__).resolve().parent.parent
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

from live_demo.config import LiveConfig, get_preset
from live_demo.audio import MelFrontEnd, MicSource, WavSource, SyntheticSource
from live_demo.engine import LiveEngine
...
def run_selftest(cfg: LiveConfig, preview_path: str) -> int:
    ...
    from live_demo.preview import render_static
...
def run_snapshot(cfg: LiveConfig, args) -> int:
    ...
    from live_demo.app import LiveDemoApp
...
def run_live(cfg: LiveConfig, args) -> int:
    ...
    win.show()
    win.start()
    print("[ live_demo ] running — keys: L learn · I inhibition · "
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--preview", default="live_demo_preview.png",
