...
class LiveConfig:
    ...
    # ---- log-frequency cochleagram channels (== model0 N) ----
    n_channels: int = 96            # log-spaced channels over [fmin, fmax]
    ...
    fmin: float = 100.0             # lowest channel centre (Hz) -- speech F0 +
                                    # low formants reach down here
    fmax: float = 8000.0            # highest channel centre (Hz)
    ...
    learn: bool = False             # plasticity off: segregation reads the cortex
                                    # ACTIVATIONS, the recurrent W is not needed
...
def default(**kw) -> LiveConfig:
    """Uniform inhibition + multi-timescale depression; plasticity off (the
    segregation reads the cortex activations, not the recurrent weights)."""
    return LiveConfig(**kw)


def uniform(**kw) -> LiveConfig:
    """Alias of default (uniform inhibition)."""
    return LiveConfig(name="uniform", inhibition="uniform", **kw)


def selective(**kw) -> LiveConfig:
    """Selective (SST-like) inhibition instead of the uniform default."""
    return LiveConfig(name="selective", inhibition="selective", **kw)
